...
根据当前项目 Harness、代码和依赖补充真实技术栈。面向小白讲解时，英文技术名词首次出现必须紧跟中文注释，例如 API（应用程序接口）、ORM（对象关系映射）。
...
> 模块讲解按“一句话作用 → 为什么项目需要 → 术语拆解 → 代码对应 → 运行链路 → 设计原因 → 易错点 → 进阶理解”展开，并将知识分成 `必须掌握`、`建议理解`、`了解即可`。

> 阶段考核固定 10 道选择题，包含单选和多选，总分 100 且每题分值不得全部相同。正确答案仅保存于本项目 `.wanan/assessment-state.json`，不得写入本文件。
'@
